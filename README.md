...
Если не указано иное, оригинальный код этого репозитория распространяется на
условиях GNU General Public License версии 3 или любой более поздней версии
(`GPL-3.0-or-later`). Полный текст GNU GPL версии 3 приведён в корневом файле
[`LICENSE`](LICENSE).

Сторонние компоненты не перелицензируются этим проектом и сохраняют собственные
уведомления об авторских правах и условия лицензирования.

### KDE plasma-nm

Каталог `nm-plugin-anet-qt6-ui/plasma-nm` подключён как Git submodule проекта
KDE `plasma-nm`. Его файлы сохраняют исходные уведомления об авторских правах и
условия лицензирования. Актуальную информацию следует смотреть непосредственно
в репозитории `plasma-nm`.

### Anet

VPN-клиент [Anet](https://github.com/ZeroTworu/anet) является отдельным проектом
и не входит в состав этого репозитория. Anet распространяется по лицензии MIT.
Этот проект взаимодействует с Anet как с отдельной программой и не включает и
не перелицензирует его исходный код.
